...
- (void)deleteImageWithIndex:(NSUInteger)index
{
    if([self.delegate respondsToSelector:@selector(deleteImageWith:atIndex:)])
        [self.delegate deleteImageWith:self atIndex:index];
    if([_photos count] == 1) {
