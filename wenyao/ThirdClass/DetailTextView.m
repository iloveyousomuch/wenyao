...
-(void)setKeyWordText:(NSString *)keyWord AndColor:(UIColor *)keyWordColor
{
    //NSRange range = [self.text rangeOfString:keyWord];
    NSRange r = NSMakeRange(0, keyWord.length);
    if (r.length > 0) {
        [resultAttributedString addAttribute:(NSString *)(kCTForegroundColorAttributeName) value:(id)keyWordColor range:NSMakeRange(0, keyWord.length)];
    }
}
