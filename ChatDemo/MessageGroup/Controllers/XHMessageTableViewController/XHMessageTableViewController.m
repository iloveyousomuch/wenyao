...
- (void)initilzer {
	...
	messageTableView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleTopMargin;
...
//    [messageTableView addFooterWithTarget:self action:@selector(footerRereshing)];
...
//    messageTableView.footerPullToRefreshText = @"上拉可以加载更多数据了";
//    messageTableView.footerReleaseToRefreshText = @"松开马上加载更多数据了";
//    messageTableView.footerRefreshingText = @"正在帮你加载中";
...
- (void)finishRecorded {
    ...
    [self.voiceRecordHelper stopRecordingWithStopRecorderCompletion:^{
        NSString *duration = weakSelf.voiceRecordHelper.recordDuration;
        float durationTime = [duration floatValue];
        if(durationTime < 1.0f) {
            //[SVProgressHUD showErrorWithStatus:@"录制时间过短" duration:0.8];
            return;
        }
        NSLog(@"%@",weakSelf.voiceRecordHelper.recordDuration);
...
{
    XHMessageTableViewCell *cell = (XHMessageTableViewCell *)[tableView cellForRowAtIndexPath:indexPath];
    if ([cell.delegate respondsToSelector:@selector(multiMediaMessageDidSelectedOnMessage:atIndexPath:onMessageTableViewCell:)]) {
        [cell.delegate multiMediaMessageDidSelectedOnMessage:self.messages[indexPath.row] atIndexPath:indexPath onMessageTableViewCell:cell];
    }
}
